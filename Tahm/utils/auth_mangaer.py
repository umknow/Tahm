...
# import cookielib
import threading
# import urlparse  # 2019-10-23 14:25:11
import requests
...
class WeXinLogin(threading.Thread):
    ...
    def __init__(self, username=None, password=None, callback=None):
        ...
        -------
            ...
            token
        '''
        self.cookie = cookiejar.CookieJar()  # 2019-10-23 14:17:10
    ...
    def run(self):
        ...
        # print('>>>>>>>>>>>', cookies, token)
        # 持久化cookie信息
        self.__save_data(json.dumps(cookies), 'cookies.txt')
        self.__save_data(json.dumps(token), 'token.txt')
    ...
    def __splite_token(self, current_url):
        ...
        Token
        """
        _token = urlparse(current_url).query.split('&')[-1].split('=')[-1]
        return _token
    ...
    def __save_cookie(self, username, cookies):
        ...
        """
        # 实例化一个LWPcookiejar对象
        new_cookie_jar = cookiejar.LWPCookieJar(username + '.txt')
